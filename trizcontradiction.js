...
// Funciones auxiliares para usar la matriz
function getContradictionPrinciples(improvingFeature, worseningFeature) {
    if (trizContradictionMatrix.matrix[improvingFeature] && 
        trizContradictionMatrix.matrix[improvingFeature][worseningFeature]) {
        return trizContradictionMatrix.matrix[improvingFeature][worseningFeature];
    }
    return [];
}

function getPrincipleName(principleNumber) {
    if (principleNumber >= 1 && principleNumber <= 40) {
        return trizContradictionMatrix.principles[principleNumber - 1];
    }
    return "Unknown Principle";
}

function getFeatureName(featureNumber) {
    if (featureNumber >= 1 && featureNumber <= 39) {
        return trizContradictionMatrix.features[featureNumber - 1];
    }
